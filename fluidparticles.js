...
var FluidParticles = (function () {
    ...
    function FluidParticles () {
        ...
        function start(programs) {
            ...
            // Default values for particle size and color
            this.particleSizeFactor = 1.0; // Size multiplier (1.0 = default size)
            this.particleColorHue = 0.6; // Hue for HSV color (0.6 = blue, 0.0 = red)
            this.colorVariety = 0.5; // Default color variety (0.5 = medium variety)
            ...
            // Add slider for particle size
            this.sizeSlider = new Slider(document.getElementById('size-slider'), this.particleSizeFactor, 0.5, 3.0, (function (value) {
                this.particleSizeFactor = value;
                // Update sphere radius in renderer if we're currently simulating
                if (this.state === State.SIMULATING) {
                    var gridResolutionX = this.simulatorRenderer.simulator.gridResolution[0];
                    var newSphereRadius = (7.0 * this.particleSizeFactor) / gridResolutionX;
                    this.simulatorRenderer.renderer.sphereRadius = newSphereRadius;
                }
            }).bind(this));
            
            // Add slider for particle color
            this.colorSlider = new Slider(document.getElementById('color-slider'), this.particleColorHue, 0.0, 1.0, (function (value) {
                this.particleColorHue = value;
                // We'll pass this to the renderer, which will be handled in the shader
                if (this.state === State.SIMULATING) {
                    this.simulatorRenderer.renderer.particleColorHue = this.particleColorHue;
                }
            }).bind(this));
            
            // Add slider for color variety
            this.colorVarietySlider = new Slider(document.getElementById('color-variety-slider'), this.colorVariety, 0.0, 1.0, (function (value) {
                this.colorVariety = value;
                if (this.state === State.SIMULATING) {
                    this.simulatorRenderer.renderer.colorVariety = this.colorVariety;
                }
            }).bind(this));
    ...
    FluidParticles.prototype.redrawUI = function () {
        ...
        this.sizeSlider.redraw();
        this.colorSlider.redraw();
        this.colorVarietySlider.redraw();
    }
    ...
    FluidParticles.prototype.startSimulation = function () {
        ...
        var sphereRadius = 7.0 / gridResolutionX * this.particleSizeFactor;
        ...
        // Also pass the color hue to the renderer
        this.simulatorRenderer.renderer.particleColorHue = this.particleColorHue;
        this.simulatorRenderer.renderer.colorVariety = this.colorVariety;
